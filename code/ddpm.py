...
from util import np2torch,torch2np,gather_and_reshape,kernel_se,get_rkhs_proj,power_test
...
def forward_hilbert_diffusion_sample(x_0,K_chols,steps,dc,noise_rate=1.0,RKHS_projs=None,
    ...
    sqrt_alphas_bar_t = gather_and_reshape(
        values=dc['sqrt_alphas_bar'],steps=steps,x_shape=x_0.shape).to(device) # [B x 1 x 1]
    sqrt_one_minus_alphas_bar_t = gather_and_reshape(
        values=dc['sqrt_one_minus_alphas_bar'],steps=steps,x_shape=x_0.shape).to(device) # [B x 1 x 1]
    x_t_mean = sqrt_alphas_bar_t * x_0 # [B x D x L] 
    x_t_std = sqrt_one_minus_alphas_bar_t # [B x D x L]
    # Correlated noise sampling
    if noise_type == 'Gaussian':
        noise = torch.randn_like(input=x_0, device=device) # [B x D x L]
    elif noise_type == 'Uniform':
        noise = 6.0*torch.rand_like(input=x_0, device=device)-3.0 # [B x D x L]
    else:
...
def get_ddpm_loss(model,x_batch,K_chols,A=None,V=None,
                  ...
                  vel_w=1e-6,acc_w=1e-6,device='cpu'):
    """
        x_batch: [B x D x L]
        K_chols: [D x L x L]
        A: [L x L]
    ...
    x_noisy,noise = forward_hilbert_diffusion_sample(
        ...
        RKHS_projs=RKHS_projs,noise_type=noise_type, device=device
    ) # [B x D x L]
    ...
    # TODO: Need to change dt for different timesteps
    # x_t_minus_one = x_batch - 0.1 * noise_pred_unflat # [B x D x L] linspace(0, 1, 100) -> np.sqrt(0.01) = 0.1
    # x_t_minus_one = x_batch - np.sqrt(2e-1) * noise_pred_unflat # linspace(-10., 10., 100)
    x_t_minus_one = x_batch - np.sqrt(0.979) * noise_pred_unflat # linspace(0., 23., 24) -> np.sqrt(23/24) ~= np.sqrt(0.979)

    # Smoothing loss
    if V is not None:
        V_expand = V[None,None,:,:].to(device) # [1 x 1 x L x L]
        ...
        vel_loss = 0.0
    if A is not None:
        A_expand = A[None,None,:,:].to(device) # [1 x 1 x L x L]
...
def eval_hddpm_1D(
    ...
    PLOT_ANCESTRAL_STEPS=True,return_output=False):
    """
    ...
    _,correlated_noise = forward_hilbert_diffusion_sample(
        ...
        RKHS_projs=RKHS_projs,noise_type='Gaussian',device=device)
...
def ddpm_train_wrapper(
    ...
    for it in range(MAX_ITER):
        ...
        if len(x_0_train.shape) == 2:
            x_batch = torch.tile(x_0_train[None,:,:],dims=(BATCH_SIZE,1,1)).to(device) # [B x D x L]
            loss,info = get_ddpm_loss(
                ...
                l1_w=l1_w,l2_w=l2_w,huber_w=huber_w,smt_l1_w=smt_l1_w,vel_w=vel_w,acc_w=acc_w,device=device)
        else:
            idx = np.random.choice(x_0_train.shape[0],BATCH_SIZE)
            x_batch = x_0_train[idx,:,:].to(device) # [B x D x L]
        ...
        if ((it % (MAX_ITER//N_PLOT)) == 0) or (it==(MAX_ITER-1)):
            eval_hddpm_1D(
                model,dc,K_chols,RKHS_projs,times,x_0_valid,
                B=n_sample,M=8,device=device,
                RKHS_PROJECTION_EACH_X_T=RKHS_PROJECTION_EACH_X_T,
                PLOT_ANCESTRAL_STEPS=False,return_output=False)    
    ...
    outs = eval_hddpm_1D(
        ...
        B=n_sample,M=8,device=device,
        RKHS_PROJECTION_EACH_X_T=RKHS_PROJECTION_EACH_X_T,
        PLOT_ANCESTRAL_STEPS=False,return_output=True)    
 
    return train_losses, valid_losses, outs
...
def ddpm_trainer(
    ...
    train_losses, valid_losses = [], []
    y_0_train = next(iter(train_loader))[1]
    # Some infos (x_0 is either [D x L] or [M x D x L] )
    L = times.shape[0]
    if len(y_0_train.shape) == 2:
        D = y_0_train.shape[0]
    elif len(y_0_train.shape) == 3:
        M = y_0_train.shape[0]
        D = y_0_train.shape[1]
    ...
    for it in range(MAX_ITER):
        ...
        for _, y_train in train_loader:
            ...
            model.train()
            optm.zero_grad()
            t = torch.randint(0, dc['T'],(y_train.shape[0],),device=device).long() # [B]
            if len(y_train.shape) == 2:
                y_train = torch.tile(y_train[None,:,:],dims=(BATCH_SIZE,1,1)).to(device) # [B x D x L]
                loss,info = get_ddpm_loss(
                    model=model,x_batch=y_train,K_chols=K_chols,A=A,V=V,
                    t=t,dc=dc,noise_rate=1.0,
                    RKHS_projs=RKHS_projs,noise_type='Gaussian',
                    l1_w=l1_w,l2_w=l2_w,huber_w=huber_w,smt_l1_w=smt_l1_w,vel_w=vel_w,acc_w=acc_w,device=device)
            else:
                # idx = np.random.choice(y_train.shape[0],BATCH_SIZE)
                # x_batch = y_train[idx,:,:].to(device) # [B x D x L]
                loss,loss_info = get_ddpm_loss(
                    model=model,x_batch=y_train,K_chols=K_chols,A=A,V=V,
                    t=t,dc=dc,noise_rate=1.0,
                    RKHS_projs=RKHS_projs,noise_type='Gaussian',
        ...
        with torch.no_grad():
            for _, y_train in valid_loader:
                ...
                valid_loss += val_loss.item() * y_train.shape[0]
                valid_num += y_train.shape[0]

        # Printout
        N_PRINT,N_PLOT = 20,10
        if ((it % (MAX_ITER//N_PRINT)) == 0) or (it==(MAX_ITER-1)):
            print ("[%d/%d][%.2f%%] train loss:[%.3f]"%(it,MAX_ITER,100*it/MAX_ITER,train_loss / train_num))
            for key in loss_info.keys():
                print ("  [%s]:[%.3f]"%(key,loss_info[key].item()))
        
        if ((it % (MAX_ITER//N_PRINT)) == 0) or (it==(MAX_ITER-1)):
            print ("[%d/%d][%.2f%%] valid loss:[%.3f]"%(it,MAX_ITER,100*it/MAX_ITER,valid_loss / valid_num))
            for key in val_loss_info.keys():
                print ("  [%s]:[%.3f]"%(key,val_loss_info[key].item()))
        ...
        if ((it % (MAX_ITER//N_PLOT)) == 0) or (it==(MAX_ITER-1)):
            with torch.no_grad():
                _, y_valid = next(iter(valid_loader))
                y_valid = y_valid.to(device)
                eval_hddpm_1D(
                    model,dc,K_chols,RKHS_projs,times,y_valid,
                    B=n_sample,M=8,device=device,
                    RKHS_PROJECTION_EACH_X_T=RKHS_PROJECTION_EACH_X_T,
                    PLOT_ANCESTRAL_STEPS=False,return_output=False)    
    ...
    for x_test, y_test in test_loader:
        ...
        outs = eval_hddpm_1D(
            model,dc,K_chols,RKHS_projs,x_test,y_test,
            B=n_sample,M=8,device=device,
            RKHS_PROJECTION_EACH_X_T=RKHS_PROJECTION_EACH_X_T,
            PLOT_ANCESTRAL_STEPS=False,return_output=True)    
